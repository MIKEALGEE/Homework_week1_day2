...
-- SELECT * FROM movies; -- 1. Return all the data in the movies table.

-- SELECT name FROM people; -- 2. Return only the name from the people table.

-- UPDATE people
-- SET name = 'Keith Douglas'
-- WHERE name = 'Kith Douglas';
-- SELECT name FROM people; -- 3. Change name to proper spelling.

-- SELECT name FROM people -- 4. Select only your name from the database.
-- WHERE name = 'Michael Griffin';

-- DELETE FROM movies WHERE --5. delete batman begins
-- title = 'Batman Begins';

-- INSERT into people (name) VALUES ('Craig Morton'); -- 6. Create a new entry in people table.

-- INSERT into people (name) VALUES ('Pawel');
-- DELETE FROM people WHERE name = 'Pawel'; 7. Delete Pawel from the list.

-- INSERT INTO movies (title, year, show_time) VALUES ('Avengers Assemble', 2012, '00:00'); 8, Add midnight showing of Avengers

-- UPDATE movies
-- SET show_time = '18:20'
-- WHERE title = 'Guardians of the Galaxy 2'; -- 9 update movie time of gg2
...
-- DELETE FROM movies WHERE id IN (1,3,6,8); 1  delete mutliple entries

-- SELECT * FROM movies -- 2 Order films by Descending.
-- ORDER BY year DESC;
